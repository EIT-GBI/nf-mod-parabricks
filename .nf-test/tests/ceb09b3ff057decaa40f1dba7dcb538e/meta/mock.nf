...
include { PARABRICKS_MUTECTCALLER } from '/Users/pauline.fourgoux/Documents/Code/nf-dnaseq/modules/parabricks/mutectcaller/tests/../main.nf'
...
workflow {
    ...
    //run process
    PARABRICKS_MUTECTCALLER(*input)

    if (PARABRICKS_MUTECTCALLER.output){

        // consumes all named output channels and stores items in a json file
        for (def name in PARABRICKS_MUTECTCALLER.out.getNames()) {
            serializeChannel(name, PARABRICKS_MUTECTCALLER.out.getProperty(name), jsonOutput)
        }	  
      
        // consumes all unnamed output channels and stores items in a json file
        def array = PARABRICKS_MUTECTCALLER.out as Object[]
